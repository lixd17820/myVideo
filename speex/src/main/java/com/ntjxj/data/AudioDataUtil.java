...
public class AudioDataUtil {
    ...
    public static short[] spx2raw(byte[] encodedData) {
        int len = encodedData.length * decFrameSize / encodedFrameSize;
        //short[] shortRawData = new short[encodedData.length * decFrameSize / encodedFrameSize];

        short[] shortRawData = new short[len];
        ...
        //Log.e("AudioUtil", encodedData.length + "/" + shortRawData.length + "/" + dest.length + "/" + re +"/" + len);
        return shortRawData;
    }
