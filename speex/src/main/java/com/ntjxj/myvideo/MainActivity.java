...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        encoder.start();
        sender = new Sender(myHandler);
        sender.start();
        reciver = new Reciver(myHandler);
        tracker = new Tracker(myHandler);
        decoder = new DecodeAudio(myHandler);
        decoder.start();
        ...
        btnPlay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (reciver.isDown()) {
                    reciver.setDown(false);
                    btnPlay.setText("播放");
                    timeThread.setTime(false);
                    return;
                }
                reciver.setDown(true);
                reciver.start();
                btnPlay.setText("停播");
                timeThread.setTime(true);
                timeThread.start();
    ...
    private Handler timeHander = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (what == 1) {
                tvTimes.setText(String.valueOf(msg.arg1 / 10 + 1));
                if (msg.arg1 >= 200) {
                    stopRecord();
                }
            } else {
