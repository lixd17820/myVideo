...
Java_com_jd_wly_intercom_audio_Speex_open(JNIEnv *env, jobject obj, jint compression) {
    int tmp;
...
JNIEXPORT jint JNICALL Java_com_jd_wly_intercom_audio_Speex_encode
    ...
    for (i = 0; i < nSamples; i++) {
        ...
        int copy_byte = ((i + 1) * enc_frame_size > size) ? (size - (i * enc_frame_size))
                                                          : enc_frame_size;

        env->GetShortArrayRegion(lin, i * enc_frame_size, copy_byte, buffer);
        // 降噪、增益、静音检测等处理
